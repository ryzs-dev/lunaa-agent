 ...
 */
async function updateSheetStatus(sheetName, spreadsheetId, rowIndex, columnIndex, status) {
    try {
        if (columnIndex === -1) {
 ...
 * @param status - Status to update
 */
async function updateSheetStatusByTracking(sheetName, spreadsheetId, trackingNumber, status) {
    var _a;
    try {
        ...
        // Update the status
        await updateSheetStatus(sheetName, spreadsheetId, targetRow, statusCol, status);
    }
    catch (error) {
