 ...
 */
async function syncSheetsToD1(sheetName = "Test") {
    var _a;
    ...
    try {
        ...
        // Bulk insert orders
        if (orders.length > 0) {
            await (0, supabaseOrders_1.bulkInsertOrders)(orders);
        }
 ...
 */
async function syncNewOrdersOnly(sheetName = "Test") {
    var _a;
    ...
    try {
        ...
        for (let i = 1; i < rows.length; i++) {
            ...
            const existingOrder = await (0, supabaseOrders_1.getOrderByTracking)(trackingNumber);
            if (existingOrder) {
                existingOrdersCount++;
            ...
            order.tracking_number = trackingNumber;
            try {
                await (0, supabaseOrders_1.insertOrder)(order);
                newOrdersCount++;
 ...
 */
async function validateSheetStructure(sheetName = "Test") {
    try {
