...
function extractOrderFromMessage(messageBody, context) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (!orderDate && line.match(/^\d{1,2}\/\d{1,2}\/(\d{2}|\d{4})/i)) {
            ...
            let dateMatch = line.match(/^(\d{1,2}\/\d{1,2}\/(?:\d{2}|\d{4}))(\.rpt)?/i);
            if (dateMatch) {
                orderDate = formatDateToYYYYMMDD(dateMatch[1]);
                isRepeatCustomer = Boolean(dateMatch[2]);
                console.log(`   ✅ Date found: ${orderDate}, Repeat: ${isRepeatCustomer}`);
                // Check if this line also contains payment method after comma
                // e.g., "8/8/25，Cod"
                const paymentInDateLine = line.match(/[，,]\s*(cod|bank|cash|transfer)\s*$/i);
