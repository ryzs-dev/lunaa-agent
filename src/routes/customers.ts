...
customersRouter.get('/id/:id', async (req, res) => {
  ...
  const customer = await customerService.getCustomerById(id as UUID);
...
customersRouter.get('/s', async (req, res) => {
  try {
    ...
    if (search) {
      query = query.or(
        `customer_name.ilike.%${search}%,phone_number.ilike.%${search}%,fb_name.ilike.%${search}%,email.ilike.%${search}%`
      );
    }
  ...
  try {
    ...
    const avgOrdersPerCustomer =
      Object.keys(customerOrderCounts).length > 0
        ? Object.values(customerOrderCounts).reduce(
            (sum, count) => sum + count,
            0
    ...
    const avgSpendingPerCustomer =
      Object.keys(customerSpending).length > 0
        ? Object.values(customerSpending).reduce(
            (sum, amount) => sum + amount,
            0
          ) / Object.keys(customerSpending).length
        : 0;
    ...
    res.json({
      ...
      data: {
        ...
        customers: {
          ...
          newCustomerRate:
            (totalCustomers ?? 0) > 0
              ? (((newCustomers || 0) / (totalCustomers ?? 0)) * 100).toFixed(1)
              : 0,
        },
        averages: {
