    ...
    try {
      ...
      if (built.error || !built.shipment) {
        results.push({
          order_id: orderId,
          success: false,
          error: built.error ?? 'Could not build shipment payload',
        });
        continue;
      }

      const shipmentResult = await parcelDailyService.createShipment(
        built.shipment,
        orderId as UUID,
      );
      ...
      if (shipmentResult?.success === false) {
        const message =
          typeof shipmentResult.message === 'string'
      ...
      results.push({
        order_id: orderId,
        success: true,
        data: shipmentResult?.data ?? shipmentResult,
      });
    } catch (error) {
      const message =
...
// Order Line Items
orderRouter.patch('/:order_id/line-items', async (req, res) => {
  try {
    const { order_id } = req.params;

    const payload: UpdateLineItemsInput = req.body;
    ...
    const updatedOrder = await orderService.updateLineItems(
      order_id as unknown as UUID,
      payload
    );
