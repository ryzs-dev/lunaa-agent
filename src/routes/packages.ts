...
packagesRouter.get("/", async (req, res) => {
  try {
    ...
    if (search) {
      query = query.or(
        `package_name.ilike.%${search}%,package_code.ilike.%${search}%,description.ilike.%${search}%`
      );
    }
...
packagesRouter.post("/", async (req, res) => {
  try {
    const packageData: Omit<Package, "id" | "created_at" | "updated_at"> =
      req.body;
    ...
    if (packageData.package_code) {
      const existingPackage = await getPackageByCode(packageData.package_code);
      if (existingPackage) {
